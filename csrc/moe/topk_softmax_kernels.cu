...
__device__ __forceinline__ float toFloat(InputType v) {
...
template <int VPT, int NUM_EXPERTS, int WARPS_PER_CTA, int BYTES_PER_LDG, int WARP_SIZE_PARAM, typename IndType, typename InputType = half>
__launch_bounds__(WARPS_PER_CTA* WARP_SIZE_PARAM) __global__
    void topkGatingSoftmax(const InputType* input, const bool* finished, float* output,
        ...
        const int start_expert, const int end_expert, const bool renormalize)
{
    ...
    // fused half to float conversion if input is not float
    // TODO(shixian): add fp16 and fix unittest.
    if constexpr (std::is_same_v<InputType, float>) {
        using VecType = AlignedArray<float, ELTS_PER_LDG>;
        VecType* row_chunk_vec_ptr = reinterpret_cast<VecType*>(&row_chunk);
        const VecType* vec_input_ptr = reinterpret_cast<const VecType*>(thread_read_ptr);
        for (int ii = 0; ii < LDG_PER_THREAD; ++ii) {
            row_chunk_vec_ptr[ii] = vec_input_ptr[ii * THREADS_PER_ROW];
        }
    } else if constexpr (std::is_same_v<InputType, __nv_bfloat16>) {
        if constexpr (ELTS_PER_LDG >= 2) {
            using VecType = AlignedArray<__nv_bfloat16, ELTS_PER_LDG>;
            float2* row_chunk_float2_ptr = reinterpret_cast<float2*>(row_chunk);
            const VecType* vec_input_ptr = reinterpret_cast<const VecType*>(thread_read_ptr);

            for (int ii = 0; ii < LDG_PER_THREAD; ++ii) {
                VecType vec = vec_input_ptr[ii * THREADS_PER_ROW];
                int base_idx_f2 = ii * ELTS_PER_LDG / 2;
                for (int jj = 0; jj < ELTS_PER_LDG / 2; ++jj) {
                    row_chunk_float2_ptr[base_idx_f2 + jj] = __bfloat1622float2(
...
#pragma unroll
    ...
    // fused output renormalization
    if (renormalize) {
        if (thread_group_idx == 0) {
            for (int k_idx = 0; k_idx < k; ++k_idx) {
                output[k * thread_row + k_idx] = topk_vals[k_idx] / denom;
            }
        }
    }
}
...
template <typename IndType, typename InputType, typename BYTES_PER_LDG_MULTIPLE_64>
...
template<typename ComputeType, typename BYTES_PER_LDG_MULTIPLE_64>
void dispatch_topk_softmax_launcher(
    ...
    int num_tokens, int num_experts, int topk, bool renormalize, cudaStream_t stream)
{
    if (topk_indices.scalar_type() == at::ScalarType::Int) {
        vllm::moe::topkGatingSoftmaxKernelLauncher<int, ComputeType, BYTES_PER_LDG_MULTIPLE_64>(
        ...
        vllm::moe::topkGatingSoftmaxKernelLauncher<uint32_t, ComputeType, BYTES_PER_LDG_MULTIPLE_64>(
            reinterpret_cast<const ComputeType*>(gating_output.data_ptr()),
            topk_weights.data_ptr<float>(),
        ...
        vllm::moe::topkGatingSoftmaxKernelLauncher<int64_t, ComputeType, BYTES_PER_LDG_MULTIPLE_64>(
            reinterpret_cast<const ComputeType*>(gating_output.data_ptr()),
            topk_weights.data_ptr<float>(),
    ...
    bool renormalize)
{
    ...
    // for half dtype we can only do 4B loading instead of 8B to ensure
    // a row is aligned with single warp loading (for num experts not power of 2 but multiple of 64)
    if (gating_output.scalar_type() == at::ScalarType::BFloat16) {
        dispatch_topk_softmax_launcher<__nv_bfloat16, 4>(gating_output, topk_weights, topk_indices, 
            token_expert_indices, softmax_workspace, num_tokens, num_experts, topk, renormalize, stream);
    } else if (gating_output.scalar_type() == at::ScalarType::Half) {
        dispatch_topk_softmax_launcher<__half, 4>(gating_output, topk_weights, topk_indices, 
            token_expert_indices, softmax_workspace, num_tokens, num_experts, topk, renormalize, stream);
    } else if (gating_output.scalar_type() == at::ScalarType::Float) {
        dispatch_topk_softmax_launcher<float, 8>(gating_output, topk_weights, topk_indices, 
            token_expert_indices, softmax_workspace, num_tokens, num_experts, topk, renormalize, stream);
    } else {
        TORCH_CHECK(false, "Unsupported gating_output data type: ", gating_output.scalar_type(), 
            ". Supported types are: BFloat16, Half, Float!");
    }
}
